<template>
  <div id="companySurveySystem">
    ...
    <div class="con">
      ...
      <div class="item">
        <x-input :show-clear="false" title="公司:" :disabled="true" @click.native="companyFocus" v-model="company_name" placeholder="请输入公司名称…">
          <img slot="right" src="../../assets/img/chaxunicon.png" alt="">
        </x-input>
      </div>
      <div class="item">
        <x-input title="姓名:" v-model="personnel_name" placeholder="请输入姓名…">
        </x-input>
      </div>
      <div class="item">
      ...
      <!-- <div class="item">
        <x-input title="地址:" placeholder="获取位置信息…">
      ...
      <popup v-model="showPopup" position="bottom">
        <div class="popupMask">
          <div class="search-box">
            <div class="search">
              <input ref="inputs" type="text" v-model="searchValue" placeholder="请输入公司名称">
            </div>
            <div class="btn" @click="searchSure">确定</div>
          </div>
          <ul class="list">
            <li v-for="(item,index) in companyList" :key="index" @click="handleLiSure(item)">
              {{item.city+','+item.company_name}}
            </li>
          </ul>
          <div style="padding:0 20px;color:#292929;font-size:14px;opacity: 0.7;" v-show="companyList.length==0">暂无对应公司，可添加</div>
        </div>
        <!-- <group>
  ...
  export default {
    ...
    data () {
      return {
        ...
        company_id:'',
        company_name:'',
        personnel_id:this.getCookie('personnel_id'),
        personnel_name:'',
        personnel_phone:''
      }
    },
    ...
    methods:{
      ...
      searchData(){
        const params={
          ...
          company_name : this.searchValue,
        }
        this.request(this.api.queryCompanyAPI, JSON.stringify(params),'post','application/json').then(res => {
          if(res.code==200){
            // 请求公司模糊查询接口
            // {
            //   code：200，
      ...
      searchSure(){
        this.company_name=this.searchValue;
        this.showPopup=false;
      },
      submit(){
        if(!this.company_name ||!this.personnel_name){
        ...
        this.request(this.api.companyRegisteredAPI, JSON.stringify(params),'post','application/json').then(res => {
          if(res.code==200){
