...
<script>
  ...
  export default {
    ...
    created(){
      ...
      const redirect_uri = this.currentLocation + "/#/gridCommunityLogin";
    ...
    methods:{
      login(code){
        ...
        this.request(this.api.communityGridLoginAPI, JSON.stringify(params),'post','application/json').then(res => {
          if(res.code==200){
            ...
            if(res.data.registered== 0){
              this.$go('/gridCommunitySurveySystem'); // 社区录入页
            }else if(res.data.registered== 1){
              this.$go('/gridCommunitySurveyForm'); // 表单页
            }
