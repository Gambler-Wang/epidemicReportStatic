<template>
  <div id="gridCommunitySurveySystem">
		<HeaderNav title="居民健康日报注册">
      <!-- <div class="add-btn" slot="right" @click="$go('/gridCommunityHistory')">
        <div></div>
      </div> -->
		</HeaderNav>
    <div class="main">
      <div class="tip">补充必要信息后<br>开始日志记录</div>
      <div class="title">
        <!-- <span>
          <i></i>
          <i></i>
  ...
  export default {
    ...
    methods:{
      ...
      submit(){
        ...
        if(!this.personnel_name){
          this.$vux.toast.text('请填写家庭责任人姓名', 'bottom');
          return;
        }
        var _res=/^[1-9]\d*$/;
        ...
        this.request(this.api.communityGridRegisteredAPI, JSON.stringify(params),'post','application/json').then(res => {
          if(res.code==200){
            window.localStorage.setItem('data',JSON.stringify(res.data));
            window.localStorage.setItem('counts',JSON.stringify(res.data.counts));
             this.$go('/gridCommunitySurveyForm');
          }else{
            this.$vux.toast.text(res.msg, 'bottom')
          }
        });
